...
import {
  ok, zodErr, unauthorized, conflict, serverError,
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const [summary] = await Guest.aggregate([
      {
        $group: {
          _id:          null,
          totalGuests:  { $sum: 1 },
          totalHadir:   { $sum: { $cond: [{ $eq: ['$rsvp.attending', true]  }, 1, 0] } },
          totalTidak:   { $sum: { $cond: [{ $eq: ['$rsvp.attending', false] }, 1, 0] } },
    ...
    return ok({
      ...
      summary: summary ?? { totalGuests: 0, totalHadir: 0, totalTidak: 0, totalPax: 0 },
    });
  } catch (e) {
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      if (!exists) { invitationCode = candidate; break; }
    }
    if (!invitationCode) return serverError();

    const guest = await Guest.create({ ...parsed.data, invitationCode });
